/*
  WiFiClient.cpp - TCP/IP client for ESP31B, mostly compatible
...
int WiFiClient::connect(IPAddress ip, uint16_t port)
{
    ...
    //os_printf("connect: " IPSTR " %u\n", IP2STR(addr.addr), port);
    ...
    while(!_client && millis() - start_at < 2000) delay(1);
    if (_client)
        return 1;
...
size_t WiFiClient::peekBytes(uint8_t *buffer, size_t length) {
    ...
    _startMillis = millis();
    while((available() < (int) length) && ((millis() - _startMillis) < _timeout)) delay(1);
...
uint8_t WiFiClient::connected()
{
    ...
    return _client->state() == 4 || available();
}
