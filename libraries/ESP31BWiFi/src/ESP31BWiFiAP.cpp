 ...
 */
bool ESP31BWiFiAPClass::softAP(const char* ssid, const char* passphrase, int channel, int ssid_hidden) {
 ...
 */
bool ESP31BWiFiAPClass::softAPConfig(IPAddress local_ip, IPAddress gateway, IPAddress subnet) {
 ...
 */
bool ESP31BWiFiAPClass::softAPdisconnect(bool wifioff) {
    bool ret;
    struct softap_config conf;
    *conf.ssid = 0;
    *conf.password = 0;
 ...
 */
uint8_t ESP31BWiFiAPClass::softAPgetStationNum() {
 ...
 */
IPAddress ESP31BWiFiAPClass::softAPIP() {
    struct ip_info ip;
 ...
 */
uint8_t* ESP31BWiFiAPClass::softAPmacAddress(uint8_t* mac) {
 ...
 * @return String mac
 */
String ESP31BWiFiAPClass::softAPmacAddress(void) {
    uint8_t mac[6];
    char macStr[18] = { 0 };
    wifi_get_macaddr(SOFTAP_IF, mac);
