/* 
  i2s.c - Software I2S library for ESP31B
...
void ICACHE_FLASH_ATTR i2s_slc_begin(){
  ...
  I2SIE = I2SITE | I2SIFSE; //Enable only for RX EOF interrupt
...
static uint8_t _ws_pin;
static uint8_t _data_pin;
static uint8_t _bck_pin;
...
void ICACHE_FLASH_ATTR i2s_begin(uint8_t ws, uint8_t data, uint8_t bck){
  _i2s_sample_rate = 0;
  i2s_slc_begin();
  ...
  pinMode(_ws_pin, FUNCTION_0);
  pinMatrixOutAttach(_ws_pin, I2SO_WS_IDX);
  pinMode(_data_pin, OUTPUT);
  pinMode(_data_pin, FUNCTION_0);
  pinMatrixOutAttach(_data_pin, I2SO_DATA_IDX);
  pinMode(_bck_pin, OUTPUT);
  pinMode(_bck_pin, FUNCTION_0);
  pinMatrixOutAttach(_bck_pin, I2SO_BCK_IDX);
...
void ICACHE_FLASH_ATTR i2s_end(){
  i2s_slc_end();
  pinMatrixOutDetach(_ws_pin); pinMode(_ws_pin, INPUT);
  pinMatrixOutDetach(_data_pin); pinMode(_data_pin, INPUT);
  pinMatrixOutDetach(_bck_pin); pinMode(_bck_pin, INPUT);
}
